...
function createClusters(mes, db="Pubmed", additionalTable="None") {
  ...
  for (var featID in mes.feature) {
    ...
    console.log(wordInfo)
    try{
      tokenData = {'word':mes.feature[featID], 'freq':mes.frequency[featID], 'group':mes.group[featID], 'tag':wordInfo['tag']}
    }catch{
            ...
            tagCanvas.querySelectorAll('.tag-cloud-item').forEach(function (element) {
                ...
                try {
                    ...
                    try {
                      ...
                      var wordLink = ""
                      if (wordTag < 0) {
                          var wordInfo2 = tagTypeColorLink_noID.find(obj => {
                              return obj.tag == wordTag
                          })
                          var wordColor = wordInfo2['color']
                          var wordLink = wordInfo2['link'] + cloudWord.replace(/\s+/g, '%20')
                      }else{
                          var wordColor = "#807689"
