...
function(input, output, session) {
  ...
  observeEvent(input$run_analysis, {
    ...
    output$biomed_terms <- DT::renderDataTable({
      DT::datatable(textstat_frequency(articles_dfm)[,c(1,2,4)],#H αγκύλη μόνο στην pubmed
    ...
    #articles_dfm_tfidf <-topfeatures(articles_dfm_tfidf)
    
    #From articles_dfm_tfidf select only features where tf-idf > cutoff
    #articles_dfm_tfidf <-  dfm_trimabs(articles_dfm_tfidf, min = 0.5)
